...
class UserController extends BaseController
{
    ...
    public function postRegister()
    {
        ...
        if ($v->fails()) {
            ...
            try {
                ...
                //success!  -- now we need to send the correct message
                $currentUser = Sentry::getUser();

                if ($currentUser->hasAccess('admin')) {
                    //send email with link to activate.
                    Mail::send('emails.auth.welcome', $data, function ($m) use ($data) {
                        $m->to($data['email'])->subject('Welcome to the international-edu');
                    });

                    Session::flash('success', 'The account has been created. An email notification has been sent to the user - with a confirmation link that will allow them to both confirm and activate their account.');
                } else {
                    //send email with link to activate.
                    Mail::send('emails.auth.welcome', $data, function ($m) use ($data) {
    ...
    public function getEdit($id)
    {
        try {
            ...
            if ($currentUser->hasAccess('admin')) {
                ...
                $systemUser = User::find($data['user']->getId());
                $data['userSchools'] = $systemUser->schools;
    ...
    /* ** WORKING - update user schools ** */
    /**
     * Process changes to user's group memberships
     * @param  int $id The affected user's id
     * @return [type]     [description]
     */
    public function postUpdateschools($id)
    {
        try {
            //Get the current user's id.
            Sentry::check();
            ...
            if ($currentUser->hasAccess('admin')) {
                ...
                // Seed the status message.
                $statusMessage .= "User now associated with school(s): ";

                foreach ($allSchools as $school) {
                    if (isset($schools[$school->id])) {
                        // add school to items the user will be associated with,
                        // and to message returned to user
                        $schoolsToAdd[] = $school->id;
                        $statusMessage .= "<br/> - " . $school->name . " , id: " . $school->id;
                    }
                }

                // SYNC - this overwrites school association for this user with new set.
                $user->schools()->sync($schoolsToAdd);

                // Inform user if this update results in user not being associated with ANY school.
                if (count($schoolsToAdd) == 0) {
                    $statusMessage = "User is no longer associated with ANY schools.";
                }
